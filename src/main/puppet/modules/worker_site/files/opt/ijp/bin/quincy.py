...
# quincy has one jobtype option

parser.add_option("--boojum", dest="boojum",
                  help="something about registration.ignore_registration_residuals_error")

(options, args) = parser.parse_args()

jobName = os.path.basename(sys.argv[0])

# Reconstruct command-line from job-specific options for inner calls
if options.boojum:
    passDownArgs = ["--boojum=" + options.boojum] + args[1:]
else:
    passDownArgs = args[1:]    
...
cmd = [QUINCY, runcfg] + passDownArgs
...
arguments = " ".join([runcfg] + passDownArgs)
